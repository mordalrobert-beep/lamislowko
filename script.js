...
async function validateGuess(word) {
  const api = await validateWordInDictionary(word);

  if (gameState.mode === GAME_MODES.NORMAL) {
    if (!api.available) {
      return {
        accepted: false,
        message: "Brak połączenia ze słownikiem. Uruchom grę przez start.py."
      };
    }
    if (!api.valid) {
      return { accepted: false, message: "Tego słowa nie ma w słowniku." };
    }
  ...
  if (!api.available) {
    return {
      accepted: false,
      message: "Brak połączenia ze słownikiem. Dla trybu obcych wpisz hasło z puli gry."
    };
  }

  return { accepted: false, message: "Tego słowa nie ma w słowniku ani w puli wyrazów obcych." };
}
